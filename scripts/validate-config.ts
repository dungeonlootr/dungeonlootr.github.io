...
const externalUrls = [
  ...
  homePage.hero.secondaryLink?.url,
  ...homePage.sections.flatMap((section) => section.externalLinks?.map((link) => link.url) ?? []),
  ...allPages.flatMap((page) => page.sections.flatMap((section) => section.externalLinks?.map((link) => link.url) ?? [])),
...
if (integrations.bannerAds) {
  const { desktop, mobile } = integrations.bannerAds;
  if (
    desktop.width !== 728 || desktop.height !== 90 || mobile.width !== 320 || mobile.height !== 50
    || !desktop.key || !mobile.key || !desktop.scriptUrl.startsWith("https://") || !mobile.scriptUrl.startsWith("https://")
  ) {
    fail("Responsive Banner advertising requires the configured HTTPS 728x90 and 320x50 placements");
  }
}

const rawAdScript = process.env.NEXT_PUBLIC_ADSTERRA_NATIVE_SCRIPT_URL?.trim();
if (integrations.socialBar && !integrations.socialBar.scriptUrl.startsWith("https://")) {
